				...
				if ( '' !== $this->get_instance_value( 'review_content_length' ) ) {
					...
					if ( count( $words ) > $content_length ) {
						...
						if ( '' !== $this->get_instance_value( 'read_more' ) ) {
							$the_content .= '<a href="' . esc_url( apply_filters( 'uael_business_reviews_read_more', $review['review_url'] ) ) . '"  target="_blank" rel="noopener noreferrer" class="uael-reviews-read-more">' . wp_kses_post( $this->get_instance_value( 'read_more' ) ) . '</a>';
						}
					}
				}
				?>
