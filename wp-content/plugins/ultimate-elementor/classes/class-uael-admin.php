	...
	final class UAEL_Admin {
		...
		public static function uael_admin_enqueue_scripts( $hook ) {
			...
			if ( isset( $branding['plugin']['short_name'] ) && '' !== $branding['plugin']['short_name'] ) {
				...
				$custom_css .= 'content: "' . esc_html( $short_name ) . '"; }';
		...
		public static function render_styles() {
			if ( isset( $_REQUEST['page'] ) && 'uaepro' === sanitize_text_field( wp_unslash( $_REQUEST['page'] ) ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Reading page slug for script enqueue, no form processing.
				add_action( 'admin_enqueue_scripts', __CLASS__ . '::enqueue_react_scripts' );
			}

			if ( ( isset( $_GET['post_type'] ) && 'elementor-hf' === sanitize_text_field( wp_unslash( $_GET['post_type'] ) ) && // phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Reading query params for script enqueue, no form processing.
					( 'edit.php' === $GLOBALS['pagenow'] || 'post.php' === $GLOBALS['pagenow'] || 'post-new.php' === $GLOBALS['pagenow'] ) ) ||
					( isset( $_GET['post'] ) && 'post.php' === $GLOBALS['pagenow'] && isset( $_GET['action'] ) && 'edit' === sanitize_text_field( wp_unslash( $_GET['action'] ) ) && 'elementor-hf' === get_post_type( absint( $_GET['post'] ) ) ) // phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Reading query params for script enqueue, no form processing.
				) {
				add_action( 'admin_enqueue_scripts', __CLASS__ . '::enqueue_react_scripts' );
			}
		...
		public static function enqueue_react_scripts() {
			...
			$branding_data      = UAEL_Helper::get_white_labels();
			$is_whitelabel      = ( ! empty( $branding_data['plugin']['name'] ) || ! empty( $branding_data['agency']['author'] ) );
		...
		public static function initialize_ajax() {
			...
			add_action( 'wp_ajax_uael_save_onboarding_analytics', __CLASS__ . '::save_onboarding_analytics' );
		}
		...
		public static function license_activation() {
			...
			if ( method_exists( BSF_License_Manager::instance(), 'bsf_process_license_activation' ) ) {
				...
				if ( class_exists( '\UAEL_Analytics_Events' ) ) {
					...
					\UAEL_Analytics_Events::track( 'license_activated', 'yes', array( 'days_since_install' => (string) $days_since_install ) );
				}
		...
		public static function license_deactivation() {
			...
			if ( method_exists( BSF_License_Manager::instance(), 'process_license_deactivation' ) ) {
				...
				// Track license deactivation event.
				if ( class_exists( '\UAEL_Analytics_Events' ) ) {
					$install_time       = get_option( 'uae_usage_installed_time', 0 );
					$days_since_install = 0;
					if ( $install_time > 0 ) {
					...
					\UAEL_Analytics_Events::track( 'license_deactivated', 'yes', array( 'days_since_install' => (string) $days_since_install ) );
				}
		 ...
		 * @return void
		 */
		public static function save_onboarding_analytics() {
			check_ajax_referer( 'uael-widget-nonce', 'nonce' );
			...
			$allowed_steps = array( 'welcome', 'configure', 'success' );
			...
			update_option( 'uaepro_onboarding_analytics', $analytics_data, false );

			wp_send_json_success( esc_html__( 'Onboarding analytics saved.', 'uael' ) );
		}
	}
