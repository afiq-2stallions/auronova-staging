...
class UAEL_Helper {
	...
	public static function get_local_time( $format = 'Y-m-d h:i:s A' ) {
		$tz_offset       = isset( $_COOKIE['GetLocalTimeZone'] ) ? (int) sanitize_text_field( wp_unslash( $_COOKIE['GetLocalTimeZone'] ) ) : 0; // phpcs:ignore WordPressVIPMinimum.Variables.RestrictedVariables.cache_constraints___COOKIE
		$tz_offset       = max( -720, min( 840, $tz_offset ) ); // Validate range: -12h to +14h in minutes.
		$timezone_name   = timezone_name_from_abbr( '', $tz_offset * 60, false );
		$local_time_zone = ! empty( $timezone_name ) ? str_replace( 'GMT ', 'GMT+', $timezone_name ) : date_default_timezone_get();
